...
class ESXiDVSwitch:
    ...
    def _parse_portgroups(self, lines: list[str], start_line: int) -> None:
        ...
        for line in range(start_line, len(lines)):
            if lines[line].startswith("DVS Name"):
                break
            if lines[line].startswith("  DVPort ID"):
                capture = True
                continue
    ...
    def set_portgroup_vlan(self, name: str, vlan: int) -> None:
        ...
        if vlan != 4095:
            ...
            command = f"LC_ALL=en_US.UTF-8 nsxdp-cli vswitch vlan policy set --guest-tagging -p {name} -dvs {self.name}"  # noqa: E501
