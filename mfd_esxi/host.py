...
class ESXiHypervisor:
    ...
    def find_dswitch(self, name: str = None, uplink: str = None, portgroup: str = None) -> "ESXiDVSwitch":
        """
        ...
        for dswitch in self.dswitch:
            if name == dswitch.name or uplink in dswitch.uplinks.values() or portgroup in dswitch.portgroups:
                return dswitch
        if name:
            raise ESXiNotFound(f"Could not find DSwitch with name {name}")
